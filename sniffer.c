...
static struct sockaddr_in6 *  sniffer_getifaddr6(pcap_if_t *iface);
/* Returns 0 on success and -1 on error. */
...
{
	pcap_addr_t *paddr;
	for(paddr = iface->addresses; paddr; paddr = paddr->next) {//erro
		struct sockaddr *saddr = paddr->addr;
...
static int sniffer_openpcap_bpf(struct sniffer *s, pcap_if_t *iface) /* {{{ */
{
	...
	if(ip != UINT32_MAX) {
		if(inet_ntop(AF_INET, &ip, addr, INET_ADDRSTRLEN)) {
			sprintf(bpfstr4, "((dst host %s) && (icmp || udp))", addr);
			hasdst4 = 1;
		}
	}
	ipv6 = sniffer_getifaddr6(iface);
	if(ipv6 != NULL) {
		if(inet_ntop(AF_INET6, &ipv6->sin6_addr, addr6, INET6_ADDRSTRLEN)) {
			sprintf(bpfstr6, "((dst host %s) && (icmp6 || udp))", addr6);
			hasdst6 = 1;
		}
	}

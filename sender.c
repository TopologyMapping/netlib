...
static uint16_t sender_compute_icmp_payload(uint16_t icmpsum, uint16_t icmpid,
		uint16_t icmpseq, uint8_t ipType);
...
struct sender * sender_create(const char *device, int ipType) /* {{{ */
{
	...
	if (sender->ipType == 4){
        ...
        sender->ipv6 = libnet_get_ipaddr6(sender->ln);
	}
	sender->icmptag = 0;
	sender->iptag = 0;
	sender->tmptag = 0;
		...
		size_t padding)
{
	...
	pload[cnt-1] = sender_compute_icmp_payload(icmpsum, icmpid, icmpseq, s->ipType);
...
static uint16_t sender_compute_icmp_payload(uint16_t icmpsum, /*{{{*/
		uint16_t icmpid, uint16_t icmpseq, uint8_t ipType)
{
	int payload;
	if (ipType == 4){
...
static struct packet * sender_make_packet(struct sender *s)/*{{{*/
{
	...
	struct packet *pkt = packet_create_ip(buf, iplen + iclen, s->ipType);
	free(buf);
	return pkt;
}/*}}}*/

struct packet * sender_send_icmp6(struct sender *s,
		...
		size_t padding)
{
    ...
    s->icmptag = libnet_build_icmpv6_echo(ICMP_ECHO, 0,
            SENDER_AUTO_CHECKSUM, icmpid, icmpseq,
            // (uint8_t *)(&payload), sizeof(uint16_t),
            (uint8_t *)pload, cnt * sizeof(uint16_t),
            s->ln, s->icmptag);
    ...
    s->iptag = libnet_build_ipv6(0, 0, sz, 0, ttl, s->ipv6, dst_ipv6,  NULL, 0, s->ln, s->iptag);
    if(s->iptag == -1) goto out;
...
struct packet * sender_send_icmp6_fixrev(struct sender *s,
		struct libnet_in6_addr dst_ipv6, uint8_t ttl, uint16_t ipid,
		uint16_t icmpsum, uint16_t rev_icmpsum, uint16_t icmpseq,
		size_t padding)
{
    ...
    struct libnet_icmpv6_hdr outer;
    outer.icmp_type = ICMP_TIMXCEED;
    outer.icmp_code = ICMP_TIMXCEED_INTRANS;
    outer.icmp_sum = htons(rev_icmpsum);
    //outer.hun.gateway = 0;

    struct libnet_ipv6_hdr iip;
    iip.ip_hl = 5;
//        iip.ip_v = 4;
//        iip.ip_tos = SENDER_TOS;
    iip.ip_len = htons(LIBNET_IPV6_H + LIBNET_ICMPV6_ECHO_H + 2);
//        iip.id = htons(ipid);
//        iip.ip_off = SENDER_FRAG;
    iip.ip_hl = 1;
//        iip.ip_p = IPPROTO_ICMP;
//        iip.ip_sum = 0;
    iip.ip_src = s->ipv6;
    iip.ip_dst = dst_ipv6;
    int chksum = libnet_in_cksum((uint16_t *)&iip, LIBNET_IPV6_H);
    //iip.ip_sum = LIBNET_CKSUM_CARRY(chksum);

    struct libnet_icmpv6_hdr iicmp;
    iicmp.icmp_type = ICMP_ECHO;
    iicmp.icmp_code = 0;
    iicmp.icmp_sum = htons(icmpsum);
    iicmp.id = 0;
    iicmp.seq = htons(icmpseq);

    assert(LIBNET_ICMPV6_H == LIBNET_ICMPV6_ECHO_H);
    uint8_t buf[LIBNET_IPV6_H + 2*LIBNET_ICMPV6_ECHO_H];
    memcpy(buf, &outer, LIBNET_ICMPV6_ECHO_H);
    memcpy(buf + LIBNET_ICMPV6_ECHO_H, &iip, LIBNET_IPV6_H);
    memcpy(buf + LIBNET_ICMPV6_ECHO_H + LIBNET_IPV6_H, &iicmp,
            LIBNET_ICMPV6_ECHO_H);
    chksum = libnet_in_cksum((uint16_t *)buf, sizeof(buf));
    iicmp.id = LIBNET_CKSUM_CARRY(chksum);

    // logd(LOG_DEBUG, "IP chksum: 0x%04x\n", ntohs(iip.ip_sum));
    // logd(LOG_DEBUG, "ICMP chksum: 0x%04x\n", ntohs(iicmp.icmp_id));

    icmpid = ntohs(iicmp.id);


    return sender_send_icmp6(s, dst_ipv6, ttl, ipid, icmpsum, icmpid, icmpseq, padding);
}
